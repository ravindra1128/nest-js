...
let EventsService = class EventsService {
    constructor(firebaseApp, uploadService, sendEmailService) {
    ...
    async getCategories() {
        try {
            logger_1.logger.info('=====getCategories request initiated====');
            ...
            categories.forEach((doc) => {
                categoriesArray.push(Object.assign({ id: doc.id }, doc.data()));
            });
            logger_1.logger.info('Categories fetched successfully');
            return (0, response_1.api_response)(categoriesArray, 200);
        }
        catch (error) {
    ...
    async getBadges() {
        try {
            logger_1.logger.info('=====getBadges request initiated====');
            ...
            const badgesArray = [];
            badges.forEach((doc) => {
                badgesArray.push(Object.assign({ id: doc.id }, doc.data()));
            });
            logger_1.logger.info('Badges fetched successfully');
            return (0, response_1.api_response)(badgesArray, 200);
        }
        catch (error) {
            logger_1.logger.error(error);
            return (0, response_1.api_response)(error, 400);
    ...
    async createEvent(user, payload) {
        try {
            logger_1.logger.info('=====createEvent request initiated====');
            const { uid } = user;
            ...
            if (userRecord) {
                ...
                const eventPayload = Object.assign(Object.assign({}, payload), { owner: uid, createdAt: new Date(), updatedAt: new Date() });
                const event = await this.firebaseApp
                    .firestore()
                    .collection('events')
                    .add(Object.assign({}, eventPayload));
                ...
                return (0, response_1.api_response)(event, 201, 'Event created successfully');
            }
        }
        catch (error) {
            logger_1.logger.error(error);
    ...
    async updateEvent(eventId, user, payload) {
        try {
            logger_1.logger.info('=====udpateEvent request initiated====');
            const { uid } = user;
            const userRecord = await this.firebaseApp.getAuth().getUser(uid);
            if (userRecord) {
                if (!userRecord) {
                ...
                const event = await this.firebaseApp
                    ...
                    .collection('events')
                    .doc(eventId);
                event.update(Object.assign({}, payload));
                logger_1.logger.info('Event updated successfully');
                return (0, response_1.api_response)(event, 201, 'Event updated successfully');
            }
        }
        catch (error) {
            logger_1.logger.error(error);
    ...
    async createEventTicket(user, payload) {
        try {
            logger_1.logger.info('=====createEventTicket request initated====');
            const { uid } = user;
            const userRecord = await this.firebaseApp.getAuth().getUser(uid);
            if (userRecord) {
                if (!userRecord) {
                ...
                const newEventTicket = await this.firebaseApp
                    ...
                    .add(Object.assign(Object.assign({}, payload), { event_id: event.id }));
                const getQrCode = await this.uploadService.uploadQrCode(newEventTicket.id, process.env.EVENT_QRCODE_BUCKET_NAME, false);
                if (getQrCode) {
                    ...
                    logger_1.logger.info('Event ticket created successfully');
                    return (0, response_1.api_response)(allData, 201, 'Event ticket created successfully');
                }
            }
        }
        catch (error) {
    ...
    async getSingleEvent(eventId) {
        try {
            logger_1.logger.info('=====getSingleEvent request initiated====');
            let eventResponse = {};
            const event = await this.firebaseApp
                .firestore()
                .collection('events')
            ...
            return (0, response_1.api_response)(eventResponse, 200);
        }
        catch (error) {
            logger_1.logger.error(error);
            return (0, response_1.api_response)(error, 400);
    ...
    async saveItem(itemId, payload) {
        try {
            ...
            const item = await this.firebaseApp
                ...
                .collection(itemType)
                .doc(itemId);
            logger_1.logger.info('Item fetched successfully');
            ...
            if (itemValues && itemValues.data() !== undefined) {
                ...
                item.update(Object.assign(Object.assign({}, item), { save_count: +1 }));
                if (saveItem) {
                    logger_1.logger.info('Item Saved Successfully');
                    (0, response_1.api_response)(saveItem, 201, 'Item Saved Successfully');
                }
            }
            else {
                logger_1.logger.info(`No item found with ${itemId}`);
                (0, response_1.api_response)([], 200, `No item found with ${itemId}`);
            }
        }
        catch (error) {
            logger_1.logger.error(error);
            (0, response_1.api_response)(error, 400);
        }
    }
    async shareItem(itemId, payload) {
        try {
            const itemType = payload.item_type;
            const item = await this.firebaseApp
                .firestore()
            ...
            if (itemValues) {
                ...
                item.update(Object.assign(Object.assign({}, item), { save_count: +1 }));
                if (shareItem) {
                    logger_1.logger.info('Item shared Successfully');
                    (0, response_1.api_response)(shareItem, 201, 'Item shared Successfully');
                }
            }
        }
        catch (error) {
    ...
    async sendEventInvite(eventId, payload) {
        console.log(...oo_oo(`3179482747_274_4_274_58_4`, payload, "*********payload**************"));
        try {
            const event = await this.firebaseApp
                .firestore()
                .collection("events")
                .doc(eventId);
            logger_1.logger.info('Event fetched successfully');
            console.log(...oo_oo(`3179482747_281_6_281_53_4`, event, "**********event**********"));
            const eventValues = event.get();
            console.log(...oo_oo(`3179482747_283_6_283_53_4`, eventValues, "1111111111111111111"));
            if (eventValues) {
                const result = this.sendEmailService.sendEmail(eventId, payload.email, payload.message);
                console.log(...oo_oo(`3179482747_287_8_287_51_4`, result, "******result********"));
                logger_1.logger.info('Invite sent Successfully');
                (0, response_1.api_response)([], 201, 'Invite sent Successfully');
            }
        }
        catch (error) {
            logger_1.logger.error(error);
    ...
    async contactUs(email, message) {
        try {
            ...
            (0, response_1.api_response)([], 201, 'Email sent Successfully');
        }
        catch (error) {
            logger_1.logger.error(error);
            (0, response_1.api_response)(error, 400);
